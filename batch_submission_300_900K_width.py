# -*- coding: utf-8 -*-
"""
Created on Fri Jan 23 15:44:55 2026
...
#This is an input script to run simulations using bash commands. The clusters we run on are HPC production clusters @Sandia
...
for width in np.arange(1,2.5,0.5):
    ...
    for xi in np.arange(0.2,1.2,0.2):
        ...
        for temp in np.arange(300, 900, 100):
            for dt0 in np.arange(0.0001, 0.0002, 0.0001):
            
                os.chdir(parent_dir)
                
                file = open(f"script.{xi}.{temp}.{dt0}.{width}.batch", "w")
            
                file.write(f"#!/bin/bash\n")
                file.write(f"## Do not put any commands or blank lines before the #SBATCH lines\n")
                file.write(f"#SBATCH --nodes=13\n")
                file.write(f"#SBATCH --time=36:00:00\n")
                file.write(f"#SBATCH --account=FY190140\n")
                file.write(f"#SBATCH --job-name=SiB_{xi}.{temp}.{dt0}.{width}\n")
                ...
                file.write(f"mpirun -np 15 /ascldap/users/wjbroke/lammps-stable_12Dec2018/src/lmp_mpi -in in.{xi}.{temp}.{dt0}.{width}.mxe_mt\n")
                
                file.close()
                
                os.system("export OMP_NUM_THREADS=36")
                os.system(f'sbatch script.{xi}.{temp}.{dt0}.{width}.batch')
