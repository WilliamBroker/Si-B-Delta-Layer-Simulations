...
for width in np.arange(1,2.5,0.5): #This defines the width of the delta layer. 1 unit here = 4.7nm
    for xi in np.arange(0.2,1.2,0.2):
        xi = round(xi, 2)
        xicomplement = 1-xi
        for temp in np.arange(300,900,100):
            for dt0 in np.arange(10,20,10): #This is the range of the initial timestep
                ...
                file.write(f"\n")
                file.write(f"## fix ID-fix all transport/atom nu [hopping frequency] Qm [barrier energy] dt [time step] rdcut [cutoff radius] interstitial(o subtitutional) [type atoms] rescale [1/0]\n")
                file.write(f"## Dimension analysis: [nu = 1.0e13 1/sec] [dt = 1.0e-2  sec]\n")
                file.write(f"## Dimension analysis: [nu = 10    1/psec] [dt = 1.0e10 psec] (THIS IS IMPORTANT!!! DONT MIX UNITS!!!)\n")
                file.write(f"\n")
                ...
                file.write(f"\n")
                file.write(f"dump 1 all custom 2000 dump.sim.{xi}.{temp}.{dt0}.{width}.* x y z frequency freq_force xi0 xi1 xi2 fxi0 fxi1 fxi2 fx fy fz\n")
