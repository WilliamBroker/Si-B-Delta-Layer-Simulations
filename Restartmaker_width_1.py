# -*- coding: utf-8 -*-
"""
Created on Fri Jan 23 15:55:00 2026
...
@author: wjbroke
"""
#This runs the restart files made from the original input script, thus keeping the simulation going.
...
Freq0 = 50.0
width = 1.0
inputdt = 0.0001 #Make sure you change this value to match the input dt value so that the restart file is read correctly
...
for xi in np.arange(0.2,1.2,0.2):
    ...
    for temp in np.arange(300,900,100):
        for dt0 in np.arange(10,20,10):
            ...
            file = open(f"inrestart.{xi}.{temp}.{dt0}.{width}.mxe_mt", "w")
            ...
            file.write(f"read_restart restart.{xi}.{temp}.{inputdt}.{width}.*")
            file.write(f"\n")
