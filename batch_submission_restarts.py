...
for xi in np.arange(0.2,1.2,0.2):
    ...
    for temp in np.arange(300, 900, 100):
        for dt0 in np.arange(50, 150, 50):
            ...
            file = open(f"script.restart.{xi}.{temp}.{dt0}.batch", "w")
            ...
            file.write(f"#SBATCH --job-name=SiB_restart{xi}.{temp}.{dt0}\n")
            ...
            file.write(f"mpirun -np 15 /ascldap/users/wjbroke/lammps-stable_12Dec2018/src/lmp_mpi -in inrestart.{xi}.{temp}.{dt0}.mxe_mt\n")
            
            file.close()
            
            os.system("export OMP_NUM_THREADS=36")
            os.system(f'sbatch script.restart.{xi}.{temp}.{dt0}.batch')
            
            print(f'Job inrestart.{xi}.{temp}.{dt0}.mxe_mt submitted')
