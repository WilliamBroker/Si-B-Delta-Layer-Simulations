...
* Inputmaker_width.py: This is a file that creates input scripts for an adjustable range of atomic molar fraction (xi) values, temperatures, and timestep (dt) values.
* Restartmaker_width.py: This is a file that creates restart input scripts for an adjustable range of atomic molar fraction (xi) values, temperatures, and timestep (dt) values. Restart scripts run previous simulations where they left off, using the "write_restart" command in LAMMPS.
